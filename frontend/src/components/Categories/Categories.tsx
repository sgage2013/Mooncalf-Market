...
function CategoryPage() {
  ...
  const currentCategory: ICategory | undefined = allCategories.find(
    (category: ICategory) => category.id === parseInt(categoryId || "", 10),
  );
              ...
              <ul>
                {subcategories.map((subCategory: ISubCategory) => (
                  <li key={subCategory.id}>
                    <NavLink
                      to={`/category/${categoryId}/${subCategory.id}/items`}
            ...
            <div className="items-grid">
              {categoryItems.items.map((item: IItem) => (
                <div key={item.id} className="item">
                  <NavLink
                    to={`/category/${categoryId}/${item.subCategoryId}/items/${item.id}`}
                  >
                    <img
